...
Accepted — 2026-09-04（"索引按项目共享"经严格风险评估确认无正确性缺陷；补 D11 内容所有权边界 / D12 按项目串行锁 / slug 改名策略三项护栏后定稿）
...
| D3 | 隔离键 `workspace_id = {tenant_id}:{project_slug}`；无项目的会议级摄入退化 `{tenant_id}:mtg-{meeting_id}` | 兑现 ADR-017 D5 预留（`projects.slug` 作 Qdrant/LightRAG 隔离键）；租户前缀命中 P8 红线；LightRAG 原生 `is_tenant=True` 查询过滤直接复用 | 仅按 meeting 隔离：回到重复理解、浪费 token 的老路 |
...
| D10 | 索引生命周期：项目级索引常驻（随 `projects` 表，ADR-017 Phase 2）；会议级临时索引随会议删除清理；图/KV 存储用 LightRAG 内建文件存储，落 `workspace/projects/{slug}/lightrag/`，**不引入图数据库** | 与 ADR-016 D2 哲学一致（规模未到不引图数据库）；文件存储可随项目目录整体备份/清理 | Neo4j 等：重型依赖，规模未到 |
...
| D12 | **按项目建索引串行化**：同一 `project_slug` 的建索引请求用分布式锁串行（复用现有 Redis，或 PG advisory lock） | LightRAG 哈希去重 + `DocStatus` 仅保证单批内幂等；跨请求并发在同一 workspace 的 `merge_nodes_and_edges` 与文件图存储上可能竞态（重复实体/不一致边/写坏）。议题池并行会议场景下此情况大概率发生 | 并发建索引：图合并竞态、实体重复 |
...
- **ADR-017（产物链/项目/议题池）**：`projects.slug` 是本 ADR 的隔离键（D3）；ADR-017 Phase 4 的"项目级代码库理解索引"由本 ADR 实施；摄入入口复用 `routers/code.py`。
...
1. 摄入入口复用 `POST /{meeting_id}/code/ingest`，新增"建索引"开关；项目场景落 `workspace/projects/{slug}/`（依赖 ADR-017 Phase 2 的 `projects` 表，此前退化会议级）。
...
3. 清理：文档/项目删除触发 4 阶段 purge；会议删除清理会议级临时索引。
...
- `workspace_id` 以 `projects.slug` 为键：slug 改名会使旧 workspace 索引成为孤儿。缓解：把 slug 改名定义为**触发重建索引**（purge 旧 workspace + 增量重建，成本受 D6 增量机制约束），并约定 slug 改为低频管理操作。备选是改用稳定的 `project_id` 作键；当前维持 slug（与 ADR-017 D5 预留一致、人类可读便于运维排查），此取舍已记录待实施期复核。
