...
11. **新会话恢复上下文**：如果用户说"继续"/"恢复"/"上次说到哪了"，先读 `<vault_root>/conclave/project-context/current-status.md` 获取最新检查点指针，再读检查点文件恢复上下文（见 §9）。vault 根目录为本地 `.trae-cn/KnowledgeVault/`。
...
**核心机制**：Index+Raw 2 层架构，检查点写入 `<vault_root>/conclave/`（不进 Git；vault 根目录为本地 `.trae-cn/KnowledgeVault/`）。
