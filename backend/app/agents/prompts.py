...
17. **Dockerfile 必须使用国内镜像源加速**：基础镜像用 `dockerproxy.net/library/python:3.12-slim`，pip 使用 `https://pypi.tuna.tsinghua.edu.cn/simple`，apt 使用 `https://mirrors.tuna.tsinghua.edu.cn/debian`，npm 使用 `https://registry.npmmirror.com`，Alpine apk 使用 `https://mirrors.aliyun.com/alpine/`。
...
{{
  ...
  "deployable_service": {{
    ...
    "dockerfile": "Dockerfile 内容（基于 dockerproxy.net/library/python:3.12-slim，pip/apt 使用国内镜像，推荐多阶段构建）",
...
14. Dockerfile 是否使用国内镜像源（dockerproxy.net 基础镜像、清华/阿里云 pip/apt/apk/npm 源）
