...
17. **Dockerfile 必须使用国内镜像源加速**：基础镜像用 `docker.m.daocloud.io/library/python:3.12-slim`，pip 使用 `https://pypi.tuna.tsinghua.edu.cn/simple`，apt 使用 `https://mirrors.tuna.tsinghua.edu.cn/debian`，npm 使用 `https://registry.npmmirror.com`，Alpine apk 使用 `https://mirrors.aliyun.com/alpine/`。
