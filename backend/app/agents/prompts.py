...
16. **Dockerfile 必须使用国内镜像源加速**：基础镜像用 `docker.m.daocloud.io/library/python:3.12-slim`，pip 使用 `https://pypi.tuna.tsinghua.edu.cn/simple`，apt 使用 `https://mirrors.tuna.tsinghua.edu.cn/debian`，npm 使用 `https://registry.npmmirror.com`，Alpine apk 使用 `https://mirrors.aliyun.com/alpine/`。
17. **推荐多阶段构建**：用 builder 阶段安装依赖并生成产物，最终阶段只复制必要文件，减小镜像体积并加快部署。
...
{{
  ...
  "deployable_service": {{
    "title": "...",
    "description": "服务说明",
    "app_code": "完整的 Python 应用代码（FastAPI/Flask，可直接运行，包含/health端点）",
    "requirements_txt": "依赖清单（每行一个包名==版本，只含第三方包）",
    "dockerfile": "Dockerfile 内容（基于 docker.m.daocloud.io/library/python:3.12-slim，pip/apt 使用国内镜像，推荐多阶段构建）",
...
14. Dockerfile 是否使用国内镜像源（docker.m.daocloud.io 基础镜像、清华/阿里云 pip/apt/apk/npm 源）
