...
class InterventionRequest(BaseModel):
    ...
    content: str = Field(..., min_length=1, max_length=10000, description="用户输入内容（最大10000字符）")
    reply_to_id: str | None = Field(None, max_length=100, description="回复的消息 ID")
...
class QueryModelsRequest(BaseModel):
    """查询可用模型列表请求（POST body，避免 API Key 出现在 URL 中）"""

    provider: str = Field("", description="厂商ID，为空则使用默认")
    api_key: str = Field("", description="自定义API Key（BYOK），为空则使用环境变量配置")
    base_url: str = Field("", description="自定义Base URL（custom厂商时使用）")
    refresh: bool = Field(False, description="是否强制刷新缓存")


class QueryBalanceRequest(BaseModel):
    """查询LLM账户余额请求（POST body，避免 API Key 出现在 URL 中）"""

    provider: str = Field("", description="厂商ID")
    api_key: str = Field("", description="自定义API Key，为空则使用环境变量")
