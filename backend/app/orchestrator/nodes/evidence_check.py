...
async def evidence_check_node(state: MeetingState) -> MeetingState:
    ...
    prefetched = getattr(state, "prefetched_evidence", None) or getattr(state, "_prefetched_evidence", None)

    if prefetched:
        # 使用预检索结果（已由 cross_team 阶段提前完成）
        retrieval_results = [(conflict, prefetched.get(conflict.get("id", "c0"), [])) for conflict in state.conflicts]
    else:
        # 无预检索时，并行检索（兼容旧路径）
        ...
        retrieval_results = await asyncio.gather(*[_retrieve_evidence(c) for c in state.conflicts])
    ...
    async def _think_one_conflict(
        ...
        if tool_registry is not None:
            try:
                ...
                req = build_evidence_prompt(
                    conflict, evidence_chunks, anchor=anchor, available_tools=tool_registry.get_available_tools()
                )
    ...
    # 并行思考所有冲突
    think_results = await asyncio.gather(*[_think_one_conflict(c, chunks) for c, chunks in retrieval_results])
