...
async def list_requests(
    ...
    """列出网络授权申请单，可按会议和状态过滤"""
    return await list_auth_requests(meeting_id=meeting_id, status=status)


@router.get("/requests/{request_id}")
async def get_request(request_id: str) -> dict[str, Any]:
    """取单条申请详情"""
    r = await get_auth_request(request_id)
    if not r:
...
async def get_pending(meeting_id: str) -> list[dict[str, Any]]:
    """取某会议的 pending 申请"""
    return await get_pending_for_meeting(meeting_id)
...
async def review_request(request_id: str, body: ReviewRequest) -> dict[str, Any]:
    ...
    result = await review_auth_request(request_id, body.action, body.comment)
    if not result:
