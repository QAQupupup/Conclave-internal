...
class MeetingCreatingMixin(Protocol):
    ...
    async def on_meeting_creating(
        self, ctx: PluginContext, payload: dict
    ) -> Next | Fallback | None: ...
...
class MeetingCreatedMixin(Protocol):
    ...
    async def on_meeting_created(
        self, ctx: PluginContext, meeting_id: str, metadata: dict
    ) -> None: ...
...
class MeetingAccessMixin(Protocol):
    ...
    async def on_meeting_accessing(
        self, ctx: PluginContext, meeting: Any
    ) -> Next | Fallback | None: ...
