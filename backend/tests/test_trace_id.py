...
def test_log_injects_request_id(caplog):
    ...
    assert records[0].request_id == "req-log-test", f"request_id 应为 req-log-test, 实际: {records[0].request_id}"
    reset_request_id(token)
...
def test_log_injects_meeting_id(caplog):
    ...
    assert records[0].meeting_id == "mtg-456", f"meeting_id 应为 mtg-456, 实际: {records[0].meeting_id}"
