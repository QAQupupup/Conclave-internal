...
| 👥 **7 种内置角色** | 产品架构、工程、安全、UX、数据、市场、主持人，各有独立视角、风险偏好与证据偏好 |
| 🧩 **动态借调** | 主持人可按议题复杂度申请补充专家角色 |
| 🧠 **Skill 注入** | 13 份规则文件（编码规范 / UI 设计系统 / 沟通风格 / 验收标准 / 审计不变量 / 异步安全等），按场景按需注入 |
| 🔀 **LLM 回退链** | 主 provider 失败或额度耗尽时自动切换回退 provider（火山方舟 → 硅基流动 → DeepSeek → OpenAI → OpenRouter） |
| 🛡 **证据诚实** | 每条论点标注来源（文档 / 网页 / 常识 / 假设），无证据即诚实降级置信度，绝不编造引用 |
| 🎯 **五层确定性** | 参数约束、结论锁定、漂移检查、全链路追踪、自动降级兜底 |
| 🐳 **Docker 沙箱** | 代码在独立兄弟容器中安全执行，多级网络隔离，可部署到本地或远程主机 |
...
| **AI 推理** | OpenAI SDK（兼容硅基流动 / DeepSeek / 火山方舟 / OpenAI / OpenRouter） | 统一 LLM 接入，可插拔，失败自动回退 | [openai](https://platform.openai.com/) |
| **检索 / RAG** | bge-m3 向量化 · bge-reranker 重排 · Qdrant · jieba · Playwright · httpx · BeautifulSoup | 证据检索、中文分词、Web 信息采集 | [qdrant](https://qdrant.tech/) · [playwright](https://playwright.dev/) |
...
```env
# OpenAI 兼容接口（支持硅基流动、DeepSeek、火山方舟、OpenAI、OpenRouter 等；Qwen 全系不可用）
...
- API 契约工程：response_model 补齐 → OpenAPI 代码生成（类型 / 运行时校验 / 客户端）→ 契约 diff 门禁
- 方法论库：商业 / 金融 / 市场调研等分析方法论与报告模板技能化
- 借调重叠检测：能力差量裁决 + 角色归一化，防止重复专家入会
- 报告 block 扩展：图表与结构图区块
- 服务持久化与恢复
- 前端生成与挂载流水线固化
