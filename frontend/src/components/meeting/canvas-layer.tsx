 ...
 * - 洞察三 tab 共用一个画布（tab 即焦点）：默认 M 档，用户可升 L 档（持久化）。
 * - 操作回放为独立 L 档画布。
...
export function CanvasLayer({ meetingId }: { meetingId: string }) {
  ...
  const upgradeInsightsCanvas = useUIStore((s) => s.upgradeInsightsCanvas);
  ...
  return (
    <>
      <FloatingPanel
        ...
        onUpgrade={insightsCanvasSize === 'M' ? upgradeInsightsCanvas : undefined}
      >
