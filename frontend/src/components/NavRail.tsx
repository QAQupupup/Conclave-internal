...
interface NavItem { view: string; title: string; svg: JSX.Element }
...
export default function NavRail() {
  const { toggleLog } = useApp();
  const navigate = useNavigate();
  const location = useLocation();

  // 从 URL 推导当前 view（用于 active 高亮）
  const currentView = (() => {
  ...
  return (
    <nav className="nav-rail">
      {ICONS.map((it) => (
        <button
          key={it.view}
          className={`nav-rail-item ${currentView === it.view ? 'active' : ''}`}
          data-view={it.view}
          onClick={() => navigate(VIEW_ROUTES[it.view])}
          title={it.title}
        >
