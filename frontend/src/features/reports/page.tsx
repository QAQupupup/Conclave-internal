            ...
            {headers.length > 0 && (
              <thead>
                <tr className="border-b border-border-default">
                  {headers.map((h, i) => <th key={i} className="px-2 py-1.5 text-left font-medium text-text-tertiary">{h}</th>)}
                </tr>
              </thead>
            )}
            <tbody>
              {rows.map((row, ri) => (
                <tr key={ri} className="border-b border-border-soft">
                  {row.map((cell, ci) => <td key={ci} className="px-2 py-1.5 text-text-secondary">{String(cell)}</td>)}
                </tr>
              ))}
            </tbody>
          </table>
