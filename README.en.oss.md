...
| 👥 **7 built-in roles** | product architect, engineering, security, UX, data, market, moderator — each with its own perspective, risk appetite, and evidence preference |
| 🧩 **Dynamic borrowing** | the moderator can request extra expert roles based on topic complexity |
| 🧠 **Skill injection** | 13 rule files (coding conventions / UI design system / communication style / acceptance criteria / audit invariants / async safety, etc.), injected on demand per scenario |
| 🔀 **LLM fallback chain** | when the primary provider fails or runs out of quota, automatically switches to a fallback (Volcengine Ark → SiliconFlow → DeepSeek → OpenAI → OpenRouter) |
...
| 🐳 **Docker sandbox** | code runs safely in isolated sibling containers with multi-tier network isolation, deployable to local or remote hosts |
...
| 🔗 **Artifact chain** | deliverables and issues are archived into Git; meeting outputs stay fully traceable |
...
| **AI inference** | OpenAI SDK (SiliconFlow / DeepSeek / Volcengine Ark / OpenAI / OpenRouter compatible) | unified, pluggable LLM access with automatic fallback | [openai](https://platform.openai.com/) |
| **Retrieval / RAG** | bge-m3 embeddings · bge-reranker reranking · Qdrant · jieba · Playwright · httpx · BeautifulSoup | evidence retrieval, Chinese tokenization, web scraping | [qdrant](https://qdrant.tech/) · [playwright](https://playwright.dev/) |
...
```env
# OpenAI-compatible endpoint (SiliconFlow, DeepSeek, Volcengine Ark, OpenAI, OpenRouter, etc.; Qwen models are NOT supported)
...
- API contract engineering: full response_model coverage → OpenAPI code generation (types / runtime validation / clients) → contract diff gate
- Methodology library: business / finance / market-research analysis methodologies and report templates as skills
- Borrow-overlap detection: capability-delta arbitration + role normalization to prevent duplicate experts
- Report block extensions: chart and diagram blocks
