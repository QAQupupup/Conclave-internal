...
| [`docs/iteration-2-design.md`](./docs/iteration-2-design.md) | 迭代二详细设计：三层记忆/动态角色库/事件回放/力导向图 | 做迭代二开发时 |
...
- **v2** ✅：三层记忆、动态角色库、力导向图、事件总线增量回放、run 异步化、审计端点。62 个测试通过。
...
迭代一（主闭环）与迭代二（系统化升级）均已完成。下一步可选方向：
